...
таймзоне пользователя. Проверки даты из раздела 6.6 — не раньше
`opened_on` счёта и не позже завтрашнего дня — требуют даты открытия
счёта в `AccountDTO`, а её там на этапе 1 нет. Кнопка появится вместе
с этим полем. Шаг `recurring` диалога дохода появится на этапе 4
