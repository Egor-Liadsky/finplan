"""Общие шаги диалогов расхода и дохода, параметризованные `DialogScope`.

`docs/architecture.md`, раздел 6.3, «Диалог дохода»: диалог дохода повторяет
диалог расхода с категориями вида `income`, включая шаг комментария; шаги —
`amount`, `category`, `account`, `confirm`, `comment` — общие, меняются только
вид операции, группа состояний FSM и фабрика `callback_data`. Раздел 6.7
...
Модуль не содержит `Router`: `routers/expense.py` и `routers/income.py`
регистрируют хендлеры со своими фильтрами (`StateFilter(AddExpense)` /
`StateFilter(AddIncome)`, `ExpenseCallback` / `IncomeCallback`) и вызывают
функции отсюда с `EXPENSE_SCOPE` / `INCOME_SCOPE`. Модуль лежит отдельно от
обоих роутеров, чтобы `income.py` не зависел от `expense.py` (и наоборот).
