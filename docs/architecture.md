...
│       │   ├── clock.py            SystemClock: реализация порта Clock, UTC
│       │   ├── db/
...
│       │   │   └── routers/
│       │   │       ├── start.py        /start, /help, привязка аккаунта, онбординг
│       │   │       ├── common.py       /cancel, /menu, главное меню, навигация nav:*
│       │   │       ├── undo.py         /undo: сторно последней операции из бота
│       │   │       ├── quick.py        Быстрый ввод свободным текстом
│       │   │       ├── expense.py      Диалог добавления расхода
│       │   │       ├── income.py       Диалог добавления дохода
│       │   │       ├── ledger_dialog.py Общие шаги диалогов расхода и дохода, без Router
...
│       │   │       └── fallback.py     Всё, что не поймали другие роутеры
│       │   ├── worker/
...
| `income.router` | `Command('income')`, `F.data.startswith('inc:')`, `StateFilter(AddIncome)` | Диалог дохода |
...
| `reports.router` | `Command('today')`, `Command('month')`, `F.data.startswith('rep:')` | Короткие сводки |
...
| `fallback.router` | всё остальное | Сообщение — подсказка «не понял, /help»; нажатие кнопки без обработчика — `answer_callback_query` с текстом «Кнопка устарела», чтобы у клиента не висели часики |

**Регистрация.** `start.router` подключается к диспетчеру напрямую:
онбордингу пользователь из базы не нужен. Все остальные роутеры
подключаются в таблице выше в указанном порядке внутрь одного
родительского роутера `registered`, на котором висит `UserMiddleware`.
Так незарегистрированный пользователь с любым апдейтом, кроме `/start`,
`/help` и шагов онбординга, получает подсказку «Нажмите /start», а
роутеры после `start` получают `user` в `data` гарантированно, без
проверки на `None`. Модули роутеров, чьи хендлеры появятся позже внутри
этапа, существуют с пустым `Router` и уже подключены: подзадачи, которые
наполняют их хендлерами, не правят `main.py`.

**Команды посреди диалога.** Хендлеры шагов диалога не принимают
текст, начинающийся с `/`. Поэтому команда, отправленная посреди
диалога, доходит до своего роутера. `/cancel`, `/start` и команды входа
в диалог (`/expense`, `/income`) сбрасывают текущее состояние.
Остальные команды — `/menu`, `/help`, `/undo`, `/balance`, `/today`,
`/month` — отвечают, не трогая состояние, и пользователь продолжает
диалог с того же шага.

**Middleware** (в порядке применения):

1. `LoggingMiddleware` — на уровне апдейта: `update_id` и `telegram_id`
   в контекст логов, `service=bot`; контекст очищается после апдейта.
2. `ErrorMiddleware` — на уровне апдейта: перехват ошибок и ответ
   текстом по разделу 12.2. Ошибки ввода, доменные ошибки и ошибки
   `application` превращаются в текст из `formatters/errors.py`.
   Остальные исключения пишутся в лог с трейсом, а пользователь
   получает «Не удалось сохранить. Попробуйте ещё раз. Код: <update_id>».
3. `ThrottlingMiddleware` — не более 20 апдейтов в минуту на
   пользователя, счётчик в Redis; превышение — тихий сброс апдейта.
   Появляется на этапе 2 вместе с Redis.
4. `UserMiddleware` — внешний middleware роутера `registered` для
   сообщений и нажатий кнопок: загрузка `User` по `telegram_id`, при
   отсутствии — ответ «Нажмите /start» без передачи апдейта дальше.
   Кладёт `user` (`UserDTO`) в `data`.

`ErrorMiddleware` стоит раньше `UserMiddleware`, а не последним, как
казалось бы естественно. Причина — первое обращение к базе на каждом
апдейте делает поиск пользователя. Если бы `ErrorMiddleware` стоял
после, недоступная база давала бы молчащего бота, а раздел 12.2 этого
не допускает.

Зависимости хендлеры получают из `Container` (раздел 2.2, `container.py`).
Диспетчер кладёт контейнер в `workflow_data` под ключом `container`, и
хендлер принимает его аргументом. Отдельной DI-библиотеки нет: объектов
на весь процесс — десяток, а их граф статичен.
...
**Онбординг.** `/start` от пользователя, которого нет в базе, ведёт
через четыре шага и заканчивается вызовом use case `RegisterUser`. Всё
собранное до этого вызова лежит в данных состояния FSM, в базу ничего
не пишется. `/start` от уже зарегистрированного пользователя
онбординг не запускает: бот здоровается и показывает главное меню.

1. `base_currency` — inline-кнопки справочника валют раздела 3.1:
   RUB, USD, EUR.
2. `timezone` — inline-кнопки одиннадцати таймзон России, по одной на
   смещение от UTC+2 до UTC+12: `Europe/Kaliningrad`, `Europe/Moscow`,
   `Europe/Samara`, `Asia/Yekaterinburg`, `Asia/Omsk`,
   `Asia/Krasnoyarsk`, `Asia/Irkutsk`, `Asia/Yakutsk`,
   `Asia/Vladivostok`, `Asia/Magadan`, `Asia/Kamchatka`. На кнопке —
   город и смещение, в callback — имя IANA.
   Вместо кнопки можно прислать имя IANA текстом. Хендлер проверяет его
   по `zoneinfo.available_timezones()`, а при промахе повторяет шаг с
   подсказкой-примером.
3. `account_type` — inline-кнопки трёх типов первого счёта: `cash`
   («Наличные»), `card` («Карта»), `bank_account` («Счёт в банке»).
   Имя счёта отдельным шагом не спрашивается: им становится подпись
   кнопки. Переименование — на этапе 2, в SPA.
4. `opening_balance` — текущий остаток текстом, в синтаксисе суммы
   быстрого ввода без знака: `15000`, `15 000`, `15k`, `1200,50`.
   Кнопка «Начать с нуля» даёт остаток 0. Отрицательный начальный
   остаток в онбординге не вводится.

После шага 4 состояние очищается, пользователь получает сообщение о
созданном счёте, reply-клавиатуру раздела 6.7 и подсказку о быстром
вводе. `/cancel` посреди онбординга сбрасывает его, и следующий `/start`
начинает заново: пользователя в базе ещё нет.

Отвергнутый вариант — брать таймзону из `language_code` апдейта. Код
языка не определяет таймзону: для `ru` это одиннадцать поясов. А
ошибка в таймзоне сдвигает `occurred_on` операций (раздел 3.3), по
которому группируются все сводки.
...
**Шаги диалога и состояния FSM.** `subcategory` на диаграмме — не
отдельное состояние, а второй уровень клавиатуры в состоянии
`category`. Нажатие категории с потомками перерисовывает клавиатуру
её дочерними категориями, кнопкой «‹ Назад» к верхнему уровню и
кнопкой выбора самой родительской категории целиком: операцию можно
отнести и к узлу, а не только к листу. Шаг счёта показывает только
неархивные счета; если такой счёт один, шаг пропускается.

**Диалог дохода** повторяет диалог расхода с категориями вида
`income`, включая шаг комментария: у дохода он нужен не реже, чем у
расхода, например «зарплата за сентябрь». Поэтому в `AddIncome` есть
состояние `comment`.

**Что из диаграммы откладывается на этапе 1.** Кнопки «Дата» на шаге
подтверждения нет, операция диалога датируется сегодняшним днём в
таймзоне пользователя. Проверки даты из раздела 6.6 диалогу поэтому
не нужны: на этапе 1 счёт создаёт только онбординг, и его
`opened_on` — сегодняшний день в таймзоне пользователя. Поле
`opened_on` в `AccountDTO` есть, его берёт быстрый ввод, где дату
пишет пользователь. Кнопка «Дата» в диалоге на этапе 1 не делается.
Шаг `recurring` диалога дохода появится на этапе 4
вместе с регулярными правилами и `recurring.router`.
...
| Сумма > 0 и < 1e12 | «Сумма должна быть больше нуля и меньше триллиона» |
...
Граница между парсером и хендлером. Парсер — чистая функция от строки и
сегодняшней даты в таймзоне пользователя. Счетов, категорий и базы он не
знает, поэтому проверяет только то, что видно из самой строки: сумма
разбирается в `Decimal` и лежит в `(0, 1e12)`, дата синтаксически
корректна, комментарий не длиннее 500 символов. Сумму парсер возвращает
без округления. Всё, что зависит от выбранного счёта, проверяет хендлер
после разрешения счёта: округление до `minor_unit` его валюты с
предупреждением, дату не раньше `opened_on`, дату не позже завтрашней.
Трёхбуквенное слово считается валютой, только если его код есть в
справочнике. Иначе это слово комментария: «-50 bar» — расход 50 с
комментарием «bar», а не ошибка.

Сообщения бота отправляются с `parse_mode=HTML`, который задаётся в
`bot.py`. Любой текст пользователя — комментарий, имена счетов и
категорий — форматтеры экранируют через `html.escape`. Сами форматтеры
возвращают `str` и aiogram не импортируют. Словарь в `errors.py` —
...
**Меню по этапам.** Меню показывает только те кнопки, за которыми уже
есть обработчик, иначе нажатие упирается в `fallback`. На этапе 1 в
главном меню три пункта: «Добавить» с «Расход» и «Доход», «Счета» с
«Остатки по счетам», «Отчёты» с «Сегодня» и «Этот месяц». Остальные
пункты схемы появляются вместе с этапами, которые их реализуют.
Переходы между уровнями меню принадлежат `common.router` и
закодированы как `nav:<пункт>`: `nav:menu`, `nav:add`, `nav:acc`,
`nav:rep`. Конечные кнопки принадлежат роутеру, который выполняет
действие, и начинаются с его префикса из раздела 6.2: `exp:new`,
`inc:new`, `acc:balance`, `rep:today`, `rep:month`. Кнопки онбординга
используют префикс `onb:` и обрабатываются `start.router`. Кнопки
reply-клавиатуры приходят обычным текстом и перехватываются
`common.router` раньше `quick.router`: «📊 Меню» открывает главное
меню, «➕ Операция» — подменю «Добавить».

**Соглашение по `callback_data`.** Используется `CallbackData`-фабрика
aiogram, формат `<scope>:<action>:<id>:<page>`, длина не более 64 байт.
Примеры: `exp:cat:x3Jk…:0`, `acc:show:Qm7d…`, `nav:menu`.
Идентификаторы UUID сокращаются до последних 8 байт в base64url, то
есть до 11 символов, и разрешаются по совпадению с этим хвостом среди
счетов или категорий пользователя. Полный UUID не помещается вместе с
остальными полями. Хвост берётся потому, что у UUIDv7 (ADR-010) первые
6 байт — метка времени в миллисекундах, а дефолтное дерево категорий
создаётся одной транзакцией. У таких категорий первые 8 байт
различаются лишь 12 случайными битами, и на трёх десятках категорий
вероятность совпадения — около десяти процентов. В последних 8 байтах
62 случайных бита и у UUIDv7, и у UUIDv4. Если хвост не нашёлся или
совпал у двух элементов, кнопка считается устаревшей: пользователь
получает `answer_callback_query` «Кнопка устарела», а клавиатура
перерисовывается.
