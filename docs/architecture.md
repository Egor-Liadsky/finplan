...
│       │   │   ├── middlewares/    Пользователь из апдейта, UoW, throttling, логи
...
| `source` | `str` | `bot`, `web`, `recurring`, `system` |
...
- Кросс-валютный перевод — это **две** операции `transfer`, связанные
  ...
  как `adjustment` при необходимости.
...
4. `UnitOfWorkMiddleware` — открывает UoW на время обработки апдейта,
   выставляет `app.user_id` для RLS, коммитит при успехе, откатывает
   при исключении.
5. `ErrorMiddleware` — перехват доменных ошибок, ответ понятным текстом.
